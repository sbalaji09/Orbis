...
export default function PromptComparisonView({
  ...
  const getMetricDelta = (v1: number, v2: number, inverse = false) => {
    if (v1 === 0 && v2 === 0) return { value: 0, direction: 'neutral' as const };
    const delta = ((v2 - v1) / (v1 || 1)) * 100;
    const direction = inverse
      ? delta < 0 ? 'better' : delta > 0 ? 'worse' : 'neutral'
      : delta > 0 ? 'better' : delta < 0 ? 'worse' : 'neutral';
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              <DialogPanel className="w-full max-w-7xl h-[90vh] overflow-hidden border-2 border-black bg-white shadow-[8px_8px_0_rgba(0,0,0,0.2)] flex flex-col">
                {/* Header */}
                <div className="flex items-start justify-between gap-4 px-6 py-4 border-b-2 border-black bg-gradient-to-r from-babyblue/5 to-transparent shrink-0">
                ...
                {loading && (
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-xs text-black/40 mt-1">This may take a moment</p>
                    </div>
                  </div>
                )}
                ...
                {error && (
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center text-error">
                      ...
                      <p className="text-sm font-semibold">Failed to load comparison</p>
                ...
                {!loading && !error && comparisonData && (
                  <>
                    ...
                    <div className="px-6 py-3 border-b border-black/10 bg-black/[0.02] shrink-0">
                          ...
                          <div className="flex flex-col h-full">
                            ...
                            <div className="flex-1 overflow-auto p-4 space-y-3">
                              {comparisonData.prompts.version1.sample_outputs.length === 0 ? (
                                <p className="text-xs text-black/40 text-center py-8">No outputs available</p>
                              ) : (
                                comparisonData.prompts.version1.sample_outputs.map((output, idx) => (
                                  <div key={idx} className="p-3 bg-black/[0.02] border border-black/10 rounded">
                        ...
                        <div className="h-full overflow-auto p-6">
                          <div className="max-w-4xl mx-auto">
                            <div className="flex items-center gap-2 mb-4">
                              ...
                              <h3 className="text-sm font-semibold text-foreground">AI-Generated Analysis</h3>
                            </div>
                            <div className="prose prose-sm max-w-none text-foreground/80">
                              <SimpleMarkdown content={comparisonData.llm_analysis || 'No analysis available.'} />
                            </div>
                          </div>
                        </div>
                      )}
      ...
      <div className="flex items-center justify-center gap-2">
        ...
        {delta !== 0 && (
          <span className={`text-[10px] font-semibold ${
            direction === 'better' ? 'text-success' : direction === 'worse' ? 'text-error' : 'text-black/40'
          }`}>
  ...
  return (
    <div className="space-y-2">
      {lines.map((line, idx) => {
        // Headers
        if (line.startsWith('### ')) {
          return <h3 key={idx} className="text-sm font-bold mt-4 mb-2 text-foreground">{line.slice(4)}</h3>;
        }
        if (line.startsWith('## ')) {
          return <h2 key={idx} className="text-base font-bold mt-5 mb-2 text-foreground">{line.slice(3)}</h2>;
        }
        if (line.startsWith('# ')) {
          return <h1 key={idx} className="text-lg font-bold mt-6 mb-3 text-foreground">{line.slice(2)}</h1>;
        }
        // Horizontal rule
  ...
  return (
    <>
      {parts.map((part, idx) => {
        ...
        if (part.startsWith('`') && part.endsWith('`')) {
          return <code key={idx} className="px-1 py-0.5 bg-black/5 rounded text-[10px] font-mono">{part.slice(1, -1)}</code>;
        }
